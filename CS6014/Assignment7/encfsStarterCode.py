...
class EncFS(Operations):
    ...
    def __init__(self, root):
        ...
        # Use the getpass function to read in the user's password and store it
    ...
    def getattr(self, path, fh=None):
        ...
        # print("FIXME: update so that the size reported here is the length
        #  of the decrypted data, not the physical file size!")
        ...
        if not os.path.isdir(path) or not os.path.isdir(full_path):
            ...
            # decrypt it, use the length of the decrypted content,
            # and then close it
            if path not in self.openFiles:
                fd = os.open(full_path, os.O_RDONLY)
                length = len(self.decrypt(fd, full_path))
                os.close(fd)
            # If the file is open, just set it to the length of the
            # byte array in the dictionary
            else:
...
'''
    ...
    def open(self, path, flags=os.O_RDWR):
        ...
        if not os.path.exists(full_path) or path in self.openFiles:
            return -1
        # Otherwise, read it, and store the decrypted version in in-memory dictionary
        else:
            fd = os.open(full_path, os.O_RDWR)
            self.openFiles[path] = self.decrypt(fd, full_path)
    ...
    def decrypt(self, fd, full_path):
        # When decrypting an existing file, the first 16 bytes are the salt
        # and the rest of the file is the encrypted contents
        salt = os.read(fd, 16)
        enc_data = os.read(fd, os.path.getsize(full_path))
    ...
    def read(self, path, length, offset, fh):
        ...
        # return an array-like type with the appropriate data
    ...
    def write(self, path, buf, offset, fh):
        ...
        # (which should be the number of bytes requested to be written)
        return length
