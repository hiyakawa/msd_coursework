...
//        std::cout << "exec start: " << ret[i].exec << std::endl;
	    ...
	    for (int j = first + 1; j < last; ++j) {
            if (tokens[j] == ">" || tokens[j] == "<") {
                 ...
                 */
                const std::string& fileName = tokens[j + 1];
                int fd;
    ...
    if (error) {
        ...
        for (const Command& command : ret) {
            if (command.fdStdin != STDIN_FILENO) {
                close(command.fdStdin);
            }
            if (command.fdStdin != STDOUT_FILENO) {
...
int builtins(Command command) {
    std::string input = command.exec;
    int rc = -1;

    // check if the input requested was "exit" and exit the program
    if (input == "exit") {
        exit(0);
    }

    // cd with no parameters should go to the user's home directory
    if (input == "cd") {
