...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin(string name, int cardnum)
        {
            ...
            var patronsList = from p in db.Patrons
                              select new
                              {
                                  NAME = p.Name,
                                  CARDNUM = p.CardNum
            ...
            foreach (var p in patronsList)
            {
                if (p.NAME == name && p.CARDNUM == cardnum)
                {
                    // the patron is in the table, a success
        ...
        public ActionResult AllTitles()
        {
            var query = from t in db.Titles
                        ...
                        select new
                        {
                            ...
                            title = t.Title,
                            author = t.Author,
                            //serial = t == null ? null : (from i in t.Inventory select i.Serial).ToList(),
                            serial = t.Inventory.Any() ? (from i in t.Inventory select i.Serial).ToList() : null,
        ...
        public ActionResult ListMyBooks()
        {
            ...
            var query = from p in db.Patrons
                        where p.Name == user
                        join c in db.CheckedOut on p.CardNum equals c.CardNum into Join1
                        from j1 in Join1.DefaultIfEmpty()
                        join i in db.Inventory on j1.Serial equals i.Serial into Join2
                        from j2 in Join2.DefaultIfEmpty()
                        join t in db.Titles on j2.Isbn equals t.Isbn into Join3
                        from j3 in Join3.DefaultIfEmpty()
                        select new
                        {
                            title = j3.Title,
        ...
        {
            CheckedOut checkingOut = new CheckedOut();
            checkingOut.CardNum = (uint)card;
            checkingOut.Serial = (uint)serial;
            db.CheckedOut.Add(checkingOut);
            db.SaveChanges();
        ...
        {
            CheckedOut returnedBook = new CheckedOut();
            returnedBook.CardNum = (uint)card;
            returnedBook.Serial = (uint)serial;
            db.CheckedOut.Remove(returnedBook);
            db.SaveChanges();
